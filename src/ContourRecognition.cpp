 ...
 * Used to create a rectangle on the contour via contour tracing.
 *
 * https://www.youtube.com/watch?v=UwY98217hFE
 */
Contour get_bounding_box_moore(const cv::Mat& mat, cv::Point b, cv::Point s) {
    ...
    // Helper point, which is used to test for single-pixel contours
    ...
    while (visited_starting_point != 2) {
        ...
        if (mat.at<unsigned char>(c) != 0) {
            ...
            // update start and endpoint of the bounding box for the contour
            ...
            b = c;
            c = next_clockwise_pixel(p, b);
            // we hit the single-pixel contour
            if (first_white == c) {
                break;
            }
        }
 ...
 */
Contours contour_recognition(const cv::Mat& mat, double size_percentage_threshold) {
    Contours contours;
    ...
    // map which tracks the pixels that have already been processed
    ...
    for (int j = 1; j < bordered.cols - 1; ++j) {
        for (int i = bordered.rows - 1; i >= 0; --i) {
            if (bordered.at<unsigned char>(i, j) > 0 && seen_map.at<unsigned char>(i, j) == 0) {
                Contour s = get_bounding_box_moore(bordered, backtrack, cv::Point(j, i));
                cv::Rect r = s.first;
